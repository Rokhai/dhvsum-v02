...
@if (!empty($field['value']))
    <div class="existing-file">
        ...
        <a href="#" class="file_clear_button btn btn-light btn-sm float-right" title="Clear file"
            data-filename="{{ $field['value'] }}"><i class="la la-remove"></i></a>
        <div class="clearfix"></div>
    </div>
@endif
{{-- Image Preview from Upload File --}}
<div id="image-preview" style="display: none;">
    <img src="#" alt="Image Preview" style="max-width: 200px;">
</div>
{{-- Show the file picker on CREATE form. --}}
<div class="backstrap-file {{ isset($field['value']) && $field['value'] != null ? 'd-none' : '' }}">
    <input id="uploadFile" type="file" name="{{ $field['name'] }}" data-filename="{{ $field['value'] ?? '' }}"
...
@push('after_scripts')
    <script>
        document.getElementById('uploadFile').addEventListener('change', function(event) {
            ...
            output.children[0].src = URL.createObjectURL(event.target.files[0]);
        ...
        <script>
            function bpFieldInitUploadElement(element) {
                ...
                if (fileInput.attr('data-row-number')) {
                    $('<input type="hidden" class="order_uploads" name="_order_' + fieldName + '" value="' + fileInput.data(
                ...
                fileClearButton.click(function(e) {
                    ...
                    if (fileInput.attr('data-row-number')) {
                        $("<input type='hidden' name='_clear_" + fieldName + "' value='" + fileInput.data('filename') +
